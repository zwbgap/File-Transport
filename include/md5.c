...
unsigned char cleanse_ctr = 0;  // 用于清除内存的计数器

// 初始化MD5上下文
int MD5_Init(MD5_CTX *c) {
    memset(c, 0, sizeof(*c));  // 将上下文置零
    c->A = INIT_DATA_A;  // 初始化A寄存器
    c->B = INIT_DATA_B;  // 初始化B寄存器
    c->C = INIT_DATA_C;  // 初始化C寄存器
    c->D = INIT_DATA_D;  // 初始化D寄存器
    return 1;
}
